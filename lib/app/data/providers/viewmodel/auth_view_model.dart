...
class AuthViewModel extends BaseModel {
  ...
  Stream<int> getTotalScoreStream() {
    // Kiểm tra useriid trước khi query
    if (useriid == null || useriid.isEmpty) {
      return Stream.value(
          0); // Hoặc throw Exception('User ID is null or empty');
    }
    ...
    return _firestore
      ...
      if (!snapshot.exists) {
        return 0; // Hoặc throw Exception('User document does not exist');
      }

      final data = snapshot.data();
      if (data == null || !data.containsKey('totalScore')) {
        return 0; // Hoặc null nếu muốn phân biệt
      }

      // Ép kiểu an toàn
      final totalScore = data['totalScore'];
      if (totalScore is int) {
        return totalScore;
      } else if (totalScore is num) {
        return totalScore.toInt();
      } else {
        return 0; // Hoặc throw Exception('Invalid totalScore type');
      }
    }).handleError((error) {
      // Xử lý lỗi Firestore (timeout, quyền, v.v.)
      print('Error fetching totalScore: $error');
      return 0; // Hoặc throw error để StreamBuilder xử lý
    });
  }
